	...
	uint32_t parity, uint32_t run, bool boundary_recovery)
{
	 ...
	 * unchanged for diagnostics. The shift is proportional, so it holds for
	 * the 500 and 1250 kbit/s profiles too.
	 */
	 ...
	 *     base + (k - 1) * nominal.
	 * Nearest-integer quantisation decides this directly, without searching
	 * every possible k. Polarity 0 takes the upper count on an exact
	 * boundary;
	 ...
	 * not three physical cells. Comparator movement spans more than one
	 * integer timer bin, so the rule cannot be an exact match; the cadence
	 * budget permits only the primary pass, so the correction stays local
	 * and proportional to the active profile.
	 */
					...
					uint32_t *out_bad_pairs)
{
	 ...
	 * Derived from OPENVLC_SFD_CELLS, not hardcoded: a fixed 16 would
	 * examine the first half of a 32-cell SFD instead of the preamble, and
	 * the SFD is not alternating, so the gate would reject every frame.
	 */
	if (history_count <
		...
		for (uint32_t k = 0; k < n; k++) {
			...
			if (a_sym == sym) {
				...
				if (consecutive_bad_pairs >
					 ...
					 * would discard both the valid length and the
