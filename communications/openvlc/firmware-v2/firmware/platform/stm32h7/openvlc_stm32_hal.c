...
OPENVLC_RX_HOT void openvlc_stm32_rx_comparator_poll(void)
{
	...
	while (edge_dma_tail != head) {
		...
		if (edge_burst_len >= OPENVLC_EDGE_BURST_LEN) {
			...
			/*
			 * Decode what was captured rather than discarding the burst.
			 *
			 * The payload sits at the FRONT of edge_burst[]; what overflows the end is
			 * trailing idle. Throwing the whole burst away on overflow discards a frame
			 * that is already in hand, then resyncs at the next real gap anyway.
			 *
			 * This cannot deliver a corrupt packet: a genuinely truncated burst still
			 * fails CRC and RS exactly as before. Enlarging the buffer instead would
			 * only move the cliff - it is already 64 KB.
			 */
	 ...
	 * rather than waiting for the first edge of the NEXT frame to reveal the
	 * gap. Waiting wastes the whole inter-frame idle and starts synchronous
	 * decoding while the following packet is already entering the DMA ring. TIM2 is free-running, so this timeout is precise
...
void openvlc_stm32_debug_poll(uint32_t now_ms)
{
	...
	(defined(STM32H743xx) || defined(STM32H723xx))
	{
		...
		if (now_ms - sweep_change_ms >=
			...
			sweep_change_ms = now_ms;
			sweep_ok_mark = oknow;
			openvlc_platform_log("COMP SWEEP thr=%lu thr_mv=%lu delivered=%lu c0=%lu c1=%lu duty=%lu next_thr=%lu\r\n",
					     (unsigned long)prev,
					     (unsigned long)OPENVLC_DAC_TO_MV(prev),
					     (unsigned long)delivered,
					     (unsigned long)openvlc_phy_dbg_track_cell0_end,
					     (unsigned long)openvlc_phy_dbg_track_cell1_end,
					     (unsigned long)((openvlc_phy_dbg_track_cell1_end *
							      1000u) /
						(openvlc_phy_dbg_track_cell0_end +
						 openvlc_phy_dbg_track_cell1_end + 1u)),
