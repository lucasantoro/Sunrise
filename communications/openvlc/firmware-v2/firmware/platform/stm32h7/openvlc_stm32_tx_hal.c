...
static void tx_complete_common(int dma_error)
{
	...
	if (slot >= 0 && slot < (int)OPENVLC_STM32_TX_SLOT_COUNT &&
		 ...
		 * and report only additional scheduling jitter. An absolute
		 * comparison marks every healthy frame as late.
		 */
	 ...
	 */
	{
		 ...
		 * instead would leave the line modulating without a break, so the
		 * keep-alive would land inside the frame's own burst.
		 */
...
static int tx_arm_and_start(int slot)
{
	 ...
	 * from the DMA-complete IRQ needs no cache maintenance here. Cleaning here
	 * would push a ~24 kB frame through a priority-1 interrupt and stall
	 * comparator-ring draining long enough to lose RX frames.
	 */
	 ...
	 * preload during cell N, so the next update changes the TX pin exactly on
	 * the timer boundary. DMA completion lands in the low tail because the
	 * frame builder appends 32+ low cells, so stopping there cannot clip data.
	 *
	 * CC1E is dropped while CCR1 is seeded and the DMA is rebuilt, parking the
	 * pin on its pulldown. Leaving the channel connected instead would avoid a
	 * sub-microsecond electrical runt, but it makes the pin HOLD the seed level
	 * for the whole rebuild - so whenever cell 0 is high the LED stays lit for
	 * microseconds at every frame start. That is a DC step into the receiver
	 * AGC 125 times a second, and it costs far more than the runt it removes.
	 *
	 * OPENVLC_TX_KEEP_OUTPUT_ENABLED=1 selects the alternative.
	 */
