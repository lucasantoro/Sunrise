...
/*
 * Side-by-side tally. The streaming decoder sees the SAME degraded edges as the
 * burst decoder, pushed in small chunks so the incremental path is exercised
 * rather than one big call. It does not gate the test yet: it reports, so the
 * two can be compared on identical samples before anything is promoted.
 */
static unsigned long stream_attempts;
static unsigned long stream_ok;
static unsigned long stream_mismatch;
static unsigned long burst_ok_tally;

#define STREAM_CHUNK 64u

void openvlc_stream_rx_debug(uint32_t *sfd_hits, uint32_t *len_ok,
			     uint32_t *len_bad, int32_t *last_parse);
static unsigned long fail_no_sfd;
static unsigned long fail_len;
static unsigned long fail_parse;
...
/*
 * Third column: the faithful port of the reference edges->bits stage, fed the
 * SAME degraded edges as the other two. Same byte layer, same CRC, so the only
 * variable between this column and the streaming one is the bit-recovery
 * strategy - which is exactly the comparison being asked for.
 */
...
static unsigned long ref_ok;
static unsigned long ref_mismatch;

static int campaign_decode_ref(const openvlc_runtime_config_t *cfg,
			       ...
			       const uint32_t *edges, size_t edge_count)
{
	...
	for (off = 0u; off < edge_count; off += STREAM_CHUNK) {
		size_t n = edge_count - off;

		if (n > STREAM_CHUNK)
			n = STREAM_CHUNK;
		if (openvlc_ref_rx_push(cfg, edges + off, n, &packet) ==
		    OPENVLC_OK) {
			got = true;
			break;
		}
...
static void campaign_stream_report(void)
{
	printf("stream vs burst: attempts=%lu burst_ok=%lu stream_ok=%lu "
	       "stream_mismatch=%lu\n",
	       stream_attempts, burst_ok_tally, stream_ok, stream_mismatch);
	printf("stream paths: absorbed=%lu toolong=%lu desync=%lu\n",
	       (unsigned long)openvlc_stream_dbg_absorbed,
	       (unsigned long)openvlc_stream_dbg_toolong,
	       (unsigned long)openvlc_stream_dbg_desync);
	printf("stream failures: no_sfd=%lu bad_len=%lu parse=%lu\n",
	       fail_no_sfd, fail_len, fail_parse);
#if defined(OPENVLC_RX_REFERENCE) && OPENVLC_RX_REFERENCE
	printf("reference port: ref_ok=%lu ref_mismatch=%lu anomaly=%lu "
	       "acquired=%lu\n",
	       ref_ok, ref_mismatch, (unsigned long)openvlc_ref_dbg_anomaly,
	       (unsigned long)openvlc_ref_dbg_acquired);
#endif
}

static int campaign_decode_stream(const openvlc_runtime_config_t *cfg,
				  const openvlc_packet_t *expected,
				  const uint32_t *edges, size_t edge_count)
{
	openvlc_packet_t packet = {0};
	...
	for (off = 0u; off < edge_count; off += STREAM_CHUNK) {
		...
		if (n > STREAM_CHUNK)
			n = STREAM_CHUNK;
		if (openvlc_stream_rx_push(cfg, edges + off, n, &packet) ==
		    OPENVLC_OK) {
			got = true;
			break;
		}
			   ...
			   const uint32_t *edges, size_t edge_count)
{
...
#if defined(OPENVLC_RX_STREAMING) && OPENVLC_RX_STREAMING
	{
		...
		if (!registered) {
			registered = true;
			atexit(campaign_stream_report);
		}
		sres = campaign_decode_stream(cfg, expected, edges, edge_count);
#if defined(OPENVLC_RX_REFERENCE) && OPENVLC_RX_REFERENCE
		{
			int rres = campaign_decode_ref(cfg, expected, edges,
						       edge_count);
...
int main(int argc, char **argv)
{
	...
	/*
	 * The window is preamble + SFD. preamble_gate_history() already builds
	 * the SFD from OPENVLC_SFD_BITS, so only this call site was hardcoded to
	 * the 1-byte width; with OPENVLC_SFD_BYTES=2 it pointed 16 cells short
	 * and the gate inspected the SFD instead of the preamble. Same defect
	 * class as the 16u fixed in comp_history_preamble_score().
	 */
	if (!openvlc_test_history_has_preamble(
		    preamble_gate_history(true),
		    OPENVLC_SFD_SYNC_PREAMBLE_CELLS + OPENVLC_SFD_CELLS) ||
