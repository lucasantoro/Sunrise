 ...
 *
 * 2026-08-19: this used to be duplicated - openvlc_tx_compat_frame_to_symbols()
 * had the PRBS version while openvlc_tx_compat_packet_to_oc_words(), which is
 * the encoder the STM32 actually calls, still emitted the plain alternation.
 * The PRBS warm-up therefore never reached the air. Keep the generator in one
 * place so the two paths cannot drift again.
 *
