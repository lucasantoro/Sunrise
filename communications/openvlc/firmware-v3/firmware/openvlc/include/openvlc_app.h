...
/*
 * Continuous RX. Feed raw input-capture timestamps straight from the DMA ring,
 * in whatever chunks the poll happens to see; the decoder keeps its state
 * across calls, so there is no burst, no gap segmentation and no frame
 * boundary known in advance. Packets are committed as they complete.
 */
